...
docker exec tracer-app pip list | grep -E "(uvicorn|fastapi|sqlalchemy)"
...
docker exec tracer-app bash -c "cd /app/app && python -m uvicorn main:app --host 0.0.0.0 --port 8000 --log-level debug" &
sleep 5
