...
REM Run with additional debugging
echo Starting container with debugging...
docker run -d ^
  --name tracer-app ^
  -p 8091:8091 ^
  -v "%cd%\tracer-backend\data:/app/app/data" ^
  -v "%USERPROFILE%\.command_log.jsonl:/app/app/data/.command_log.jsonl:ro" ^
...
docker exec tracer-app curl -s http://127.0.0.1:8000/health >nul 2>&1
if %errorlevel% equ 0 (
...
docker exec tracer-app curl -s http://127.0.0.1:8091/health >nul 2>&1
if %errorlevel% equ 0 (
